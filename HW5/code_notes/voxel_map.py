...
class VoxelMap():
    def __init__(self, voxels_per_side = 25, width_on_a_side = 100, initial_prob = 0.5):
        ...
        self.radius = width_on_a_side/2 #radius of voxel map
        ...
        self.pD = 0.85 # (Good detection) probability that there is something there and it will be detected
        self.pFA = 0.3 # (False Alarm) probability that will detect and there is nothing there
        self.ps = 0.9 # probability that it is filled and will stay filled
        self.pt = 0.2 # probability that something that was empty will become occupied
    ...
    def update_map(self, measurements = None):
        self.prob_map = self.ps*self.prob_map+self.pt*(1-self.prob_map) #propagate model
        if measurements is not None: #measurement update has 2 updates. Measured occupied points, and then unoccupied points in path

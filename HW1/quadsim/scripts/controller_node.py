...
import controllers.altitude_controller as alc
...
class ControllerNode:
    ...
    def __init__(self):
        ...
        self.alt_controller = self.setup_altitude_controller()
    ...
    def setup_altitude_controller(self):
        ns = 'altitude_controller/'
        kp = rospy.get_param(ns+'kp')
        kd = rospy.get_param(ns+'kd')
        ki = rospy.get_param(ns+'ki')
        throttle_eq = rospy.get_param(ns+"equilibrium_throttle", 0.5)
        max_throttle = rospy.get_param(ns+"max_throttle", 1.0)
        min_altitude = rospy.get_param(ns+"min_altitude", 1.0)
        return alc.AltitudeController(kp,kd, ki, min_altitude, throttle_eq, max_throttle)
    ...
    #     self.traj.pos = traj[:3, 0] # position in the inertial frame
    #     self.traj.vel = traj[:3, 1] # velocity in the inertial frame
    #     self.traj.accel = traj[:3, 2] # acceleration in the inertial frame
    #     self.traj.jerk = traj[:3, 3] # rate of change of acceleration inertial frame
    #     self.traj.yaw = traj[3, 0] # yaw, inertial
    #     self.traj.yaw_rate = traj[3, 1] # yaw rate, inertial
    ...
    def compute_control(self, dt):
        ...
        # Altitude Controller
        altitude_desired = 10.0
        altitude_current = -self.state.pos[2]
        print("current altitude")
        print(altitude_current)
        throttle_cmd = self.alt_controller.calc_throttle(altitude_desired,altitude_current,dt)
        print("throttle_command")
        print(throttle_cmd)
        # Attitude stabilizer
        ## TODO att_cmd inverse should happen within the traj_follower.calc_command() function
        #att_des = att_cmd.inverse()
        euler = np.array([0,0,0])
        att_des = quat.from_euler(euler)
        ang_vel_des = np.array([0,0,0])
        ang_vel_cmd = self.att_controller.calc_command(att_des, ang_vel_des,
            self.state.att)
